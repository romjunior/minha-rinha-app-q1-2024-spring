...
- Usar hash da URI no Nginx para manter as transações de um cliente na mesma API e serializá-las na aplicação com locks em stripes. O optimistic locking permanece como proteção residual.
- Cachear os clientes, pois seus limites são imutáveis no cenário da aplicação.
- Limitar as filas HTTP para caber no orçamento de memória; não aumentar timeouts como resposta a saturação.
